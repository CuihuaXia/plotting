...
my_theme <- function(base_size = 12, scale = 1, frame = "panel") {
  ...
  base <- theme_bw(base_size = base_size) +
    theme(
      ...
      ## Facet
      strip.text        = element_text(size = rel(1.05 * scale), face = "bold", margin = margin(t = 5 * scale, r = 0, b = 6 * scale, l = 0)),
      strip.background  = element_rect(fill = "grey92", colour = "black", linetype = "solid", linewidth = 0.5 * scale),
      strip.switch.pad.grid = unit(6 * scale, "pt"),
      strip.switch.pad.wrap = unit(6 * scale, "pt")
    )
